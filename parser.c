...
/***
 * Converts the inputs into a single pipeline to chain them with seq or para
 * separators For example: A | B ; C -> This will first create two single inputs
 * separated with a pipe Then, upon encountering a ";" symbol, It needs to
 * convert the first two inputs into a pipeline and turn the separator into a
...
void convert_to_pipeline(parsed_input *input) {
    ...
    for (int i = 0; i < input->num_inputs; i++)
        memcpy(pipeline1.commands[i].args, input->inputs[i].data.cmd.args,
               MAX_INPUTS * sizeof(char *));
...
/***
 * Converts a single command to pipeline after encountering pipe symbol in a
...
void convert_command_to_pipeline(parsed_input *input) {
    ...
    memcpy(pipeline1.commands[0].args, input->inputs[input_index].data.cmd.args,
           MAX_INPUTS * sizeof(char *));

    input->inputs[input_index].type = INPUT_TYPE_PIPELINE;
    memcpy(&(input->inputs[input_index].data.pline), &pipeline1,
           sizeof(pipeline));
}
 ...
 * @param is_command
 */
void write_buffer(parsed_input *input, char *buffer, int is_command,
                  int is_pipeline) {
    int input_index;
    int arg_index;
    int current_command;
    if (is_command) {
        ...
        if (input->inputs[input_index].type == INPUT_TYPE_PIPELINE) {
            current_command =
                input->inputs[input_index].data.pline.num_commands - 1;
            arg_index = 1;
            for (; input->inputs[input_index]
                       .data.pline.commands[current_command]
                       .args[arg_index];
                 arg_index++)
                ;
        } else {
            arg_index = 1;
            for (; input->inputs[input_index].data.cmd.args[arg_index];
                 arg_index++)
                ;
        }
    }
    if (input->inputs[input_index].type == INPUT_TYPE_PIPELINE) {
        input->inputs[input_index]
            .data.pline.commands[current_command]
            .args[arg_index] = (char *)calloc(strlen(buffer), sizeof(char));
        strcpy(input->inputs[input_index]
                   .data.pline.commands[current_command]
                   .args[arg_index],
               buffer);
        input->inputs[input_index]
        ...
        input->inputs[input_index].data.cmd.args[arg_index] =
        ...
        if (is_quote) {
            ...
            if (*current_char == ')') {
                ...
                if (input->separator == SEPARATOR_PARA ||
                    ...
                    fprintf(stderr, "Subshells cannot be chained with a "
            ...
            if (is_waiting_command) {
                if (*current_char == '"' || *current_char == '\'') {
                    ...
                    fprintf(stderr, "There should be a command or a pipeline "
                                    "before semicolon.\n");
                    return 0;
                } else if (*current_char == ',') {
                    fprintf(stderr, "There should be a command or a pipeline "
                    ...
                    fprintf(stderr, "There should be a command or a subshell "
                                    "before pipe.\n");
                    return 0;
                } else {
                    is_free = 0;
                ...
                if (*current_char == '"' || *current_char == '\'') {
                    ...
                    fprintf(stderr, "There cannot be a subshell after a "
                                    "command. There should be a separator.\n");
                    return 0;
                } else if (*current_char == ';') {
                    if (input->separator == SEPARATOR_PARA) {
                        fprintf(stderr, "There cannot be a sequential "
                                        "separator after parallel.\n");
                        return 0;
                    }
                    if (input->separator == SEPARATOR_PIPE) {
                        if (check_subshell(input)) {
                            fprintf(stderr, "There cannot be a sequential "
                                            "separator after a subshell.\n");
                            return 0;
                        }
                    ...
                    if (input->separator == SEPARATOR_SEQ) {
                        fprintf(stderr, "There cannot be a parallel separator "
                                        "after sequential.\n");
                        return 0;
                    }
                    if (input->separator == SEPARATOR_PIPE) {
                        if (check_subshell(input)) {
                            fprintf(stderr, "There cannot be a parallel "
                                            "separator after a subshell.\n");
                            return 0;
                        }
                        convert_to_pipeline(input);
                    ...
                    if (input->separator == SEPARATOR_PIPE) {
                        ...
                        is_waiting_arg = 0;
                               ...
                               input->separator == SEPARATOR_SEQ) {
                        int input_index = input->num_inputs - 1;
                        if (input->inputs[input_index].type ==
                            INPUT_TYPE_COMMAND) {
                ...
                if (*current_char == ';') {
                    ...
                    fprintf(stderr,
                            "Subshells should be followed by | or nothing.\n");
                    return 0;
                }
            }
            ...
            if (isspace(*current_char)) {
                ...
                if (input->separator == SEPARATOR_PARA) {
                    fprintf(stderr, "There cannot be a sequential separator "
                                    "after parallel.\n");
                    return 0;
                }
                if (input->separator == SEPARATOR_PIPE) {
...
void free_single_input(single_input *input) {
    ...
    case INPUT_TYPE_SUBSHELL:
        // Since subshell uses a fixed-size char array, there's no dynamic
        // memory to free
        break;
...
void free_parsed_input(parsed_input *input) {
    ...
    // Since inputs is a fixed-size array within the structure, no need to free
    // it separately
}
